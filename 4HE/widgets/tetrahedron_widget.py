...
class TetrahedronWidget(gl.GLViewWidget):
    ...
    def _get_font_size_for_tetra_label(self, simplicity):
        ...
        """
        # Base font size for the simplest ratio (e.g., 1:1:1:1, simplicity 4)
        base_simplicity = 4
        base_font_size = 2.0  # Increased base font size in 3D units

        # Define a scaling factor. This creates an inverse relationship:
        # as simplicity increases, font size decreases.
        # Using a logarithmic scale can make the size differences more manageable.
        # We'll cap the minimum font size to prevent labels from becoming too small.
        
        # Avoid division by zero or log of zero for very small simplicity values
        if simplicity < base_simplicity:
            simplicity = base_simplicity

        # Logarithmic scaling: font_size = base_font_size * log(base_simplicity) / log(simplicity)
        # This ensures that at base_simplicity, font_size is base_font_size.
        # As simplicity increases, font_size decreases.
        font_size = base_font_size * (math.log(base_simplicity) / math.log(simplicity))
        
        # Clamp font size to a reasonable range
        min_font_size = 0.8 # Increased minimum font size
        max_font_size = 4.0 # Increased maximum font size
