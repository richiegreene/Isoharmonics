...
def generate_ji_triads(limit_value, equave=Fraction(2,1), limit_mode="odd", prime_limit=7, max_exponent=4):
    ...
    if limit_mode == "odd" or limit_mode == "integer":
        ...
        # A rough estimation could be prime_limit * max_exponent * 3 # Heuristic
        pass
...
def generate_ji_tetra_labels(limit_value, equave_ratio, limit_mode="odd", prime_limit=7, max_exponent=4):
    ...
    and simplicity (intervallic odd limit) for the tetrahedron.
    """
    labels_data = []
    ...
    valid_numbers = set()
    if limit_mode == "odd":
        # Heuristic for max number to check. Max possible numerator/denominator in a chord.
        # If odd_limit is 9, we might have 16/9, so numbers can be up to 16.
        # A safe upper bound for numbers in the chord could be limit_value * equave_ratio_float * 2 (to account for octave shifts)
    ...
    for combo in combinations_with_replacement(sorted_valid_numbers, 4):
        ...
        # Ensure the odd limit of all intervals is within the limit_value
        ...
        # Calculate simplicity as the intervallic odd limit
